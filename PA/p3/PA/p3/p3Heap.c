...
void* balloc(int size) {
    ...
    // add bytes for header and for footer
    alloc_size = alloc_size + sizeof(header) + sizeof(footer);

    blockHeader *current = heap_start;

    while (current->size_status != 1) {
        t_begin = (void*)current;
